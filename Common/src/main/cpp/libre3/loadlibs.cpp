...
static bool sourceActivationMatches(JNIEnv *env, jclass cl, jlong time,
                                    jlong account) {
	std::array<jbyte,10> legacy{};
	if(!legacyActivationCommand(env,cl,time,account,legacy))
		return false;
	const auto source=dp::activation_command_data(time,account);
	return !memcmp(legacy.data(),source.data(),source.size());
	}

static bool verifyActivationGenerator(JNIEnv *env, jclass cl, jlong time,
                                      jlong account) {
	...
	for(const auto &test:cases) {
		if(!sourceActivationMatches(env,cl,test[0],test[1])) {
			LOGGER("Libre 3 activation generator mismatch for %" PRId64 ",%" PRId64 "\n",
			       test[0],test[1]);
			return false;
			}
		}
	return sourceActivationMatches(env,cl,time,account);
	}
...
extern "C" JNIEXPORT jint JNICALL fromjava(startTimeIDsum)(JNIEnv *env, jclass cl, jbyteArray bArr, jlong time, jlong account) {
	...
	if(!loadNFC()||!verifyActivationGenerator(env,cl,time,account)) {
		LOGSTRING("startTimeIDsum could not verify the source activation generator\n");
		return JNI_ERR;
		}
	const auto source=dp::activation_command_data(time,account);
